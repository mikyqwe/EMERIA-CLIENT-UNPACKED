...
bonus_list = [
	["PvM",
		[
			[item.APPLY_ATTBONUS_MONSTER,63],
			[item.APPLY_ATTBONUS_METIN,144],
	...
	["Elementi",
		[
			...
			[item.APPLY_RESIST_FIRE,136],
			[item.APPLY_RESIST_ELEC,137],
			[item.APPLY_RESIST_WIND,138],
			[item.APPLY_ATTBONUS_ELEC,140],
			[item.APPLY_ATTBONUS_WIND,141],
			[item.APPLY_ATTBONUS_EARTH,142],
			[item.APPLY_ATTBONUS_DARK,143],
			[item.APPLY_ATTBONUS_FIRE,144],
			[item.APPLY_ATTBONUS_ICE,145],
		]
	],
	["Generico (Attacco)",
		[
	...
	["Status",
		[
			...
			[item.APPLY_GOLD_DOUBLE_BONUS,83],
			[item.APPLY_ITEM_DROP_BONUS,83],
		]
	],
]

stat_list = [
 ["Regno Jinno", player.KILL_JINNO],
 ["Regno Shinsoo", player.KILL_SHINSO],
 ["Regno Chunjo", player.KILL_CHUNJO],
 ["Kill Complessive", player.KILL_ALLPLAYER],
 ["Duelli Vinti", player.KILL_DUELWON],
 ["Duelli Persi", player.KILL_DUELLOST],
 ["Mostri Uccisi", player.KILL_MONSTERS],
 ["Boss Sconfitti", player.KILL_BOSSES ],
 ["Rocce Metin", player.KILL_METINSTONE]
]
	...
	def __LoadScript(self):
		...
		for i in xrange(len(bonus_list)):
			...
			for j in xrange(len(bonus_data)):
				...
				bonusImage.SetStringEvent("MOUSE_OVER_IN",self.OverInBonus, bonusText)
				bonusImage.SetStringEvent("MOUSE_OVER_OUT",self.OverOutBonus)
		...
		if self.btnIndex == 0:
			...
			for i in xrange(len(stat_list)):
				if elementList.has_key("%d_stat"%i):
					elementList["%d_stat"%i].SetText(localeInfo.MoneyFormat(player.GetKillLOG(stat_list[i][1])))
