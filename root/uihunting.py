...
class HuntingRewardWindow(ui.ScriptWindow):
	...
	def OpenReward(self, level, reward, reward_count, rand_reward, rand_reward_count, money, exp):
		...
		self.showRandomItemName = False
		self.cur = [0,0,0]
		self.GetChild("ButtonRecive").SetText(localeInfo.HNT_OPEN_APRI)
	...
	def Close(self):
		if self.clickOpenButton == False:
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.RICOMPENSA_HNT)
			return
		else:
			self.GetChild("item_icon").Hide()
	...
	def ButtonReciveReward(self):
		if self.clickOpenButton == False:
			...
			self.GetChild("ButtonRecive").SetText("Concludi")
...
class HuntingMiniWindow(ui.Window):
	...
	def LoadWindow(self):
		try:
			...
			self.title_text.SetFontName(localeInfo.UI_DEF_FONT)
			...
			self.mob_text.SetText("Comandantul Jurãmântului Alb")
			self.mob_text.Show()
