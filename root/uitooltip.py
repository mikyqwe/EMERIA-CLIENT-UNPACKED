...
class ItemToolTip(ToolTip):
	...
	def SetShopItem(self, slotIndex, canSkip = False):
		itemVnum = shop.GetItemID(slotIndex)
		if 0 == itemVnum:
			return

		price = shop.GetItemPrice(slotIndex)
		moneyType = shop.GetItemMoneyType(slotIndex)
		self.ClearToolTip()
	...
	if app.ENABLE_RENDER_TARGET:
		def __AppendRenderTarget(self, itemVnum):
			if uiRenderTarget.IsCanShowItems(itemVnum):
				self.AppendSpace(5)
				self.AppendTextLine("        |Ekey_shift|e"+" + "+"|Ekey_lclick|e - Anteprima",grp.GenerateColor(0.7607, 0.7607, 0.7607, 1.0),False)
	...
	def AdditionalTips(self, window_type, itemVnum, metinSlot, slotIndex, attrSlot = 0):
		...
		if app.ENABLE_EMOJI_SYSTEM and item.IsFlag(item.ITEM_FLAG_STACKABLE) and window_type in (player.INVENTORY,):
			if player.GetItemCount(window_type, slotIndex) > 1:
				self.AppendTextLine("           |Ekey_shift|e"+" + "+"|Ekey_lclick|e - Split Item",grp.GenerateColor(0.7607, 0.7607, 0.7607, 1.0),False)
