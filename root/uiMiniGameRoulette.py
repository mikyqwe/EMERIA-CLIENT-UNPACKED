...
import ui, app, net, wndMgr, localeInfo, uiCommon, player, item
...
class RouletteWindow(ui.ScriptWindow):		
	...
	def Show(self, price):
		self.__LoadWindow()
		self.Reset()
		item.SelectItem(price)
		self.Price.SetText("x1 " + item.GetItemName())
	...
	def __LoadWindow(self):
		...
		try:			
			...
			if self.spin_button:
				...
				self.spin_button.SetOverInEvent(ui.__mem_func__(self.ButtonOver))
				self.spin_button.SetOverOutEvent(ui.__mem_func__(self.__SlotOverOutItem))
	...
	def ButtonOver(self):
		if self.tooltipItem:
			self.tooltipItem.SetItemToolTip(19)
