...
class SwitchbotWindow(ui.BoardWithTitleBar):
	...
	def __LoadWindow(self):
		...
		self.SetPosition((wndMgr.GetScreenWidth() - BOARD_WIDTH)/2 , (wndMgr.GetScreenHeight() -BOARD_HEIGHT)/2)
	...
	def __RefreshAttributeRows(self):
		for i in xrange(ATTR_SLOT_COUNT):
			...
			if app.ENABLE_FROZEN_SYSTEM:
				(applyType, applyValue, frozen) = player.GetItemAttribute(player.SWITCHBOT, self.selectedSlot, i)
			else:
