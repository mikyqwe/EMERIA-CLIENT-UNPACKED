		...
		for i in xrange(1,6):
			if self.GetVnumMaterial(i) != 0:
				...
				if app.ENABLE_SPECIAL_STORAGE_SYSTEM:
					if player.GetItemCountByVnumNew(self.GetVnumMaterial(i)) >= self.GetCountMaterial(i):
						number = player.GetItemCountByVnumNew(self.GetVnumMaterial(i)) / (self.GetCountMaterial(i))
						count_items.append(number)
...
class CubeRenewalWindows(ui.ScriptWindow):
	...
	def UpdateMaterialCount(self):
		...
		for i in xrange(1,6):
			if self.selectedItem.GetVnumMaterial(i) != 0:
				if app.ENABLE_SPECIAL_STORAGE_SYSTEM:
					if player.GetItemCountByVnumNew(self.selectedItem.GetVnumMaterial(i)) >= (self.selectedItem.GetCountMaterial(i)*count):
				...
				if app.ENABLE_SPECIAL_STORAGE_SYSTEM:
					if player.GetItemCountByVnum(self.selectedItem.GetVnumMaterial(i)) > 0 and player.GetItemCountByVnumNew(self.selectedItem.GetVnumMaterial(i)) <= 0:
						...
						self.cube_elements["count_elements_%d"%i].SetText("%d/%d"%(player.GetItemCountByVnumNew(self.selectedItem.GetVnumMaterial(i)),self.selectedItem.GetCountMaterial(i)*count))
				else:
					self.cube_elements["count_elements_%d"%i].SetText("%d/%d"%(player.GetItemCountByVnum(self.selectedItem.GetVnumMaterial(i)),self.selectedItem.GetCountMaterial(i)*count))
	...
	def OnUpdate(self):
		OPEN_CUBE_LIMIT_RANGE = 1000

		(x, y, z) = player.GetMainCharacterPosition()
		if abs(x - self.xCubeStart) > OPEN_CUBE_LIMIT_RANGE or abs(y - self.yCubeStart) > OPEN_CUBE_LIMIT_RANGE:
			self.Close()
	...
	def OverInItem(self, index):
		...
		#	self.toolTip.AddItemData(vnum,metinSlot,attrSlot,1,1)
		#else:
		self.toolTip.AddItemData(vnum,metinSlot,attrSlot)
